...
export default function DataNapi({ inmates, onNotif, onRefresh, user, onPrint }) {
        ...
        <div className="crud-header">
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            {onPrint && filtered.length > 0 && (
              <button className="crud-add-btn" style={{ background: '#2e7d32' }} onClick={() => onPrint({ inmates: filtered }, 'datanapi')}>
                🖨️ Print Semua Data
              </button>
            )}
            {user?.role === 'warden' && (
